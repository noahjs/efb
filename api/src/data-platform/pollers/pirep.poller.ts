  ...
  async execute(): Promise<PollerResult> {
    try {
      ...
      const pireps = features.map((f) => {
        ...
        p.obs_time = props.obsTime ? new Date(props.obsTime) : null;
        p.properties = props;
      ...
      return { recordsUpdated: pireps.length, errors: 0 };
    } catch (error) {
      this.logger.error(`Failed to fetch PIREPs: ${error.message}`);
      return { recordsUpdated: 0, errors: 1, lastError: error.message };
    }
  }
}
